...
class AdminDashboardModel:
    ...
    def get_kpi_counts():
        ...
        try:
            ...
            return AdminDashboardModel._fetch_kpi_counts(cursor)
        finally:
            if cursor:
                cursor.close()
            if conn:
    ...
    def get_todays_deliveries():
        ...
        try:
            conn = get_connection()
            cursor = conn.cursor(dictionary=True)
            return AdminDashboardModel._fetch_todays_deliveries(cursor)
        finally:
            if cursor:
                cursor.close()
            if conn:
    ...
    @staticmethod
    def _fetch_unpaid_deliveries(cursor):
        cursor.execute("""
            SELECT
                t.delivery_id,
            ...
            WHERE t.payment_status = 'unpaid'
              AND d.status = 'delivered'
            GROUP BY
                t.delivery_id, c.full_name,
                c.contact_number, t.total_amount
    ...
    def get_unpaid_deliveries():
        ...
        try:
            conn = get_connection()
            cursor = conn.cursor(dictionary=True)
            return AdminDashboardModel._fetch_unpaid_deliveries(cursor)
        finally:
            if cursor:
                cursor.close()
            if conn:
    ...
    def get_all_dashboard_data():
        ...
        try:
            ...
            return {
                ...
                "unpaid_deliveries": AdminDashboardModel._fetch_unpaid_deliveries(cursor),
            }
        finally:
            if cursor:
                cursor.close()
